...
class _VehicleDetailsState extends State<VehicleDetails> {
  ...
  final AddVehicleBuyer viewModel = AddVehicleBuyer(); // Instantiate your view model
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              Container(
                height: 32,
                width: 140,
                child: CustomButton(text: 'Next', onPressed: () async {
                  SharedPreferences preferences = await SharedPreferences.getInstance();
                  String userId=  preferences.getString("id").toString();
                  print("for car "+userId);
        
                  viewModel.userId.value = userId; // Replace 'user_id' with your actual user ID
                  viewModel.brand.value = brandnames[selectedBrandIndex]; // Replace 'brand' with the selected brand
                  viewModel.model.value = model.text;
                  viewModel.trim.value = trim.text;
                  viewModel.batteryCapacity.value = batterycapacity.text;
        
                  // Call the API using the view model
                  await viewModel.addBuyerVehicle(context);
